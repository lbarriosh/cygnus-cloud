...
def servers():
    ...
    if(request.args(0) == 'add_servers'):
        ...
        form = FORM(HR(),H2(T('Añadir un nuevo servidor')),DIV( T('Nombre: '),BR(),INPUT(_name = 'name')),
                ...
                HR(),CENTER(INPUT(_type='submit',_name = 'add' ,_value=T('Añadir Usuario'))))                
        
        if form.accepts(request.vars,keepvalues=True) and form.vars.add:
            if(len(form.vars.name) > 0) and (len(form.vars.password) > 0) and (len(form.vars.port) > 0):
        ...
        infoServer = request.vars.info or ['ServerDefault','IpDefault','PortDefault']
        print infoServer 
        ...
        form1 = FORM(T('Servidores'),SELECT(_name = 'server',*[OPTION('Server1',_value = 'Server1'),OPTION('Server2', _value = 'Server2')],_onchange = "ajax('%s', ['server'], 'info')" % URL('changeOption')))
        form2 = FORM(HR(),H2(T('Información del servidor')),DIV( T('Nombre: '),BR(),DIV(_id =  'info')),
                DIV( T('Dirección IP: '),BR(),LABEL(infoServer[1])),
                DIV( T('Puerto: '),BR(),LABEL(infoServer[2])),
                HR(),CENTER(INPUT(_type='submit',_name = 'add' ,_value=T('Añadir Usuario')))) 
                
        if form1.accepts(request.vars,keepvalues=True) and form1.vars.server:  
            redirect(URL(c='administrator',f='servers',args = ['remove_servers'],vars = dict(info = [form1.vars.server,'Ip1','port1']) ))
