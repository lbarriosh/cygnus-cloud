"""
    ...
    Copyright 2012-13 Luis Barrios Hernández, Adrián Fernández Hernández, Samuel Guayerbas    Martín
...
def runVM():
    ...
    if(request.args(0) == 'run'):
        ...
        if vmListAux == None:
            ...
            for l in listInfo :
                vmListAux.append([l.VMByGroup.VMId,l.Subjects.name,l.ClassGroup.cod,l.ClassGroup.curseGroup,l.UserGroup.userId])        
        ...
        i = 0
        for s in servers:
                serversSelector.append(OPTION(T(str(s["VMServerName"])),_value = s["VMServerName"] ,_selected="selected"))
                i = i + 1
        ...
        vmServerSelector = SELECT(_name = 'vmInServer',_id= 'vmInServerId')
        ...
        for vmDist in vmDistribution:
                if finalVMList.count(vmDist["ImageID"]) >= 1:
                    ...
                    vmServerSelector.append(OPTION(T("Máquina: ") + vmName + T(", Servidor: ") + vmDist["VMServerName"]
                        ,_value = vmName + "-" + vmDist["VMServerName"]  ,_selected="selected"))
                    i = i + 1 
        ...
        if form1.accepts(request.vars,keepvalues=True) and form1.vars.search:
            ...
            if len(form1.vars.vmName) > 0 :
                listSubjects = []                
                for l in listInfo :      
                    vmName = connector.getBootableImagesData([l.VMByGroup.VMId])
                    if len(vmName) > 0:
                        vmName = vmName[0]["ImageName"]
        ...
        if form2.accepts(request.vars,keepvalues=True) and form2.vars.deleteAll:
                 if form2.vars.vms:
                     #Borramos la imagen del sistema
                     commandId = connector.deleteImageFromInfrastructure(form2.vars.vms)
                     evaluateCommand(connector,commandId,"Petición de eliminación enviada")
                     
        if form2.accepts(request.vars,keepvalues=True) and form2.vars.deployAll:
                 if form2.vars.vms:
                     #Extraemos las asignaturas asociadas a esta máquina virtual
                     subjectsNumbers = userDB((userDB.VMByGroup.VMId == form2.vars.vms) & (userDB.VMByGroup.cod == userDB.ClassGroup.cod) \
                         & (userDB.VMByGroup.curseGroup == userDB.ClassGroup.curseGroup)).select(userDB.ClassGroup.placesNumber)
                     #Nos quedamos con el mayor
                     max = 0
                     for n in subjectsNumbers:
                         if n.placesNumber > max:
                             max = n.placesNumber
                     #Desplegamos la MV automáticamente
                     commandId = connector.autoDeployImage(form2.vars.vms,max)
                     evaluateCommand(connector,commandId,"Petición de desplegado automátic enviada") 
                       
        if form2.accepts(request.vars,keepvalues=True) and form2.vars.deploy:      
            #Desplegamos la máquina en un servidor concreto
            if form2.vars.vms:
                     print form2.vars.servers
                     commandId = connector.deployImage(form2.vars.servers,form2.vars.vms)         
                     evaluateCommand(connector,commandId,"Petición de desplegado en servidor enviada") 
                     
        if form3.accepts(request.vars,keepvalues=True) and form3.vars.delete:  
            #Eliminamos la máquina del servidor seleccionado
            if form3.vars.vmInServer:
        ...
        for vm in vmList:  
                if(request.args(0) == 'stop'):          
                    ...
                    TD(LABEL(vm[2])),
                    TD(LABEL(vm[3])),
                    TD(DIV(INPUT(_type='submit',_name = 'restart',  _value = T('Reiniciar'),_class="button button-blue")
                        ,_id = 'r' + str(j)),_class='izquierda'),
                    TD(DIV(INPUT(_type='submit',_name = 'stop',  _value = T('Detener'),_class="button button-blue")
                        ,_id = 's' + str(j)),_class='izquierda')))
                else:
                    table.append(TR(\
                    TD(INPUT(_type='radio',_name = 'selection',_value = vm[4],_id = "c"+str(j))),\
                    TD(LABEL(vm[0])),
        ...
        if form1.accepts(request.vars,keepvalues=True) and form1.vars.search:
            ...
            vmListAux = connector.getActiveVMsData(True,False)
            print vmListAux
...
def servers():
    ...
    if(request.args(0) == 'add_servers'):
        ...
        form = FORM(HR(),H2(T('Añadir un nuevo servidor')),DIV(T('Nombre: '),BR(),INPUT(_name = 'name',
                _style="width:63%;"),_style="position:absolute;left:12%;"),
        ...
        if serverInfo == None:
                statePercentages=dict(RAMPercentage=0,
                                    ...
                                    CPUsPercentage=0)
                table = TABLE(_class='state_table', _name='table')
                table.append(TR(TH("No Info",_class='izquierda state_table'),TH("Usado",_class='state_table'),TH("Total",_class='state_table')))
...
def users():  
    ...
    if(request.args(0) == 'add'):
        ...
        form = FORM(HR(),H2(T('Añadir un nuevo usuario')),DIV( T('Nombre: '),BR(),INPUT(_name = 'name')),
                ...
                DIV(T('Imagen de perfíl'),BR(),INPUT(_type='file',_name='imageLoader')),HR(),
                CENTER(INPUT(_type='submit',_class="button button-blue",_name = 'add' ,_value=T('Añadir Usuario'),_style="width:130px")))               
        ...
        if form.accepts(request.vars,keepvalues=True) and form.vars.add:
            if(form.vars.name != None) and (form.vars.name != "") and (form.vars.password != None) and (form.vars.password != ""):
                ...
                if(userDB(userDB.auth_user.email == form.vars.name).count() == 0):
                    ...
                    if form.vars.imageLoader != "":
                         image = userDB.userImage.file.store(form.vars.imageLoader.file, form.vars.imageLoader.filename)
                         userDB.userImage.insert(file=image,userId=form.vars.name)
        ...
        #Creamos el segundo formulario    
        form2 = FORM(H2(T('Añadir asignatura')),DIV(T('Usuario'),BR(),SELECT(_name = 'usersSelect',_id='usersSelect', *listUsers)),
              ...
              #,IMG(_src="",_style="height:90px;width:90px;",_class='profile',_name='profileImage',_id='profileImage')
               ...
               (userDB((userDB.ClassGroup.cod == form2.vars.code) & (userDB.ClassGroup.curseGroup == form2.vars.classGroup)).count() == 1):
                   if (userDB((userDB.UserGroup.cod == form2.vars.code) & (userDB.UserGroup.curseGroup == form2.vars.classGroup)).count() < \
                       ...
                       (userDB.ClassGroup.curseGroup == form2.vars.classGroup)).select(userDB.ClassGroup.placesNumber)[0].placesNumber):
                           ...
                           response.flash = T('No hay plazas libres para esta asignatura.')
               else:
        ...
        if form2.accepts(request.vars,keepvalues=True) and form2.vars.remove:
            if(form2.vars.selection != ""):
                ...
                image = userDB(listUsers[int(form2.vars.selection)]==userDB.userImage.userId).select()
                if len(image) > 0: 
                    os.remove(os.getcwd() + "/applications/CygnusCloud/uploads/" + image[0].file)                   
                    userDB(listUsers[int(form2.vars.selection)]==userDB.userImage.userId).delete()
...
def subjects():
    ...
    if(request.args(0) == 'add'):              
        #Creamos el primer formulario
        form = FORM(HR(),H2(T('Añadir un nueva asignatura')),DIV( T('Código: '),BR(),INPUT(_name = 'cod',_style="width:30%;"),_style="position:absolute;left:12%;"),
                DIV( T('Nombre: '),BR(),INPUT(_name ='name'),_style="position:absolute;left:18%;"),BR(),BR(),BR(),
                DIV( T('Año: '),BR(),INPUT(_name ='year',_style="width:35%;"),_style="position:absolute;left:12%;"),                
                DIV( T('Plazas: '),BR(),INPUT(_name ='placesNumber',_style="width:29%;"),_style="position:absolute;left:20%;"),
                DIV( T('Grupo: '),BR(),INPUT(_name ='group',_style="width:15%;"),_style="position:absolute;left:27%;")
                    ,BR(),BR(),BR(),
            ...
            if(len(form.vars.cod) >= 4) and (len(form.vars.name) != 0) and (len(form.vars.year) == 4) and (len(form.vars.curse) == 1) and \
                ...
                try:
                    ...
                    if(userDB((userDB.ClassGroup.cod == form.vars.cod) & (userDB.ClassGroup.curseGroup == form.vars.group)).count() == 0):
                        ...
                        userDB.ClassGroup.insert(yearGroup = int(form.vars.year),cod = int(form.vars.cod),curseGroup = form.vars.group,
                                placesNumber = int(form.vars.placesNumber))
                except ValueError:
...
def createSubjectsSearchForm(state):        
    ...
    form1 = FORM(HR(),H2(T('Buscar una asignatura')),DIV( T('Código: '),BR(),INPUT(_name = 'cod',_style="width:40%;"),_style="position:absolute;left:12%;"),
                ...
                DIV( T('Grupo: '),BR(),INPUT(_name ='group',_style="width:15%;"),_style="position:absolute;left:23%;"),BR(),BR(),BR(),
                INPUT(_type='submit',_class="button button-blue",_name = 'search',_value=T('Buscar')),HR())    
...
def createUserTable(listUsers):
    ...
    i = 0
    for l in listUsers:
        #Añadimos la imagen si existe
        image = userDB(l==userDB.userImage.userId).select()
        if len(image) > 0:
            table.append(TR(\
            ...
            TD(DIV(IMG(_src=URL('appadmin/download/userDB/',image[0].file),_style="height:90px;width:90px;",_class='profile')))))  
            print URL('appadmin/download/userDB/',image[0].file)    
        else:
            table.append(TR(\
            TD(INPUT(_type='radio',_name = 'selection',_value = i)),\
            TD(LABEL(l)),\
