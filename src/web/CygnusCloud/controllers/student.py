...
def createTable(subject,j):
    ...
    for l in userDB((userDB.VMByGroup.cod == subject.cod) & (userDB.VMByGroup.curseGroup == subject.curseGroup) & \
        ...
        descriptionAct = userDB((userDB.VMByGroup.cod == subject.cod) & \
        ...
        table.append(TR(TD(INPUT(_type='radio',_name = 'selection',_value = i + j,_id = "c"+str(i + j)
        ),TD(LABEL(l.name)),TD(DIV(P(descriptionAct),CENTER(INPUT(_type='submit',_name = 'run',  _value = T('Arrancar'))),_id = str(i + j))))))
        i = i + 1
